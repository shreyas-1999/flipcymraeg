...
interface CustomVocabularyPracticeProps {
  onBack: () => void
  onCreateNew?: () => void
}

export default function CustomVocabularyPractice({ onBack, onCreateNew }: CustomVocabularyPracticeProps) {
  const { user } = useAuth()
  const { practiceMode } = useSettings()
  const [flashcards, setFlashcards] = useState<CustomFlashcard[]>([])
  const [filteredFlashcards, setFilteredFlashcards] = useState<CustomFlashcard[]>([])
  ...
  const loadFlashcards = async () => {
    ...
    try {
      ...
      const uniqueCategories = [...new Set(userFlashcards.map((card) => card.category))].sort()
  ...
  const handleCorrect = async () => {
    ...
    const newPoints = Math.min(currentCard.points + 10, 50)
    const newMastered = newPoints >= 50
    const newReviewCount = currentCard.reviewCount + 1

    // Update local state
    const updatedFlashcards = [...flashcards]
    const originalIndex = flashcards.findIndex((card) => card.id === currentCard.id)
    if (originalIndex !== -1) {
    ...
    // Update in Firestore
    try {
      await customFlashcardService.updateFlashcardProgress(currentCard.id, newPoints, newMastered, newReviewCount)
    } catch (error) {
      ...
      <div className="space-y-6">
        <Card className="max-w-md mx-auto">
          <CardHeader className="text-center">
            <CardTitle>No Custom Flashcards</CardTitle>
            <CardDescription>Create your first custom flashcard to start practicing!</CardDescription>
          </CardHeader>
          <CardContent className="text-center">
            <Button onClick={onCreateNew || onBack} className="bg-blue-600 hover:bg-blue-700">
      ...
      <div className="space-y-6">
        ...
        <div className="flex items-center space-x-4">
          ...
          <Select value={selectedCategory} onValueChange={setSelectedCategory}>
            ...
            <SelectContent>
              <SelectItem value="All">All Categories ({flashcards.length})</SelectItem>
        ...
        <Card className="max-w-md mx-auto">
          <CardHeader className="text-center">
            <CardTitle>No Flashcards in "{selectedCategory}"</CardTitle>
            <CardDescription>
              {selectedCategory === "All"
                ? "You don't have any custom flashcards yet."
                : `You don't have any flashcards in the "${selectedCategory}" category.`}
            </CardDescription>
          </CardHeader>
  ...
  return (
    <div className="space-y-6">
      ...
      <Card className="max-w-2xl mx-auto">
        <CardHeader className="text-center">
          <div className="flex items-center justify-between mb-4">
            <Badge className="bg-purple-100 text-purple-800">{currentCard.category}</Badge>
            <div className="flex items-center space-x-2">
              <div className="text-sm font-semibold text-blue-600">{currentCard.points}/50 pts</div>
              {currentCard.mastered && <Star className="h-4 w-4 text-yellow-500 fill-current" />}
              <Button
                variant="outline"
                size="sm"
        ...
        <CardContent className="space-y-6">
          ...
          {practiceMode === "welsh-to-english" && (
            <div className="flex justify-center">
              <AudioPlayer text={currentCard.welsh} />
            </div>
          )}
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm text-gray-600">
              ...
              <span>{currentCard.points}/50 points</span>
            </div>
            <Progress value={(currentCard.points / 50) * 100} className="h-2" />
      ...
      <div className="text-center text-sm text-gray-600">
        Card {currentIndex + 1} of {filteredFlashcards.length} in{" "}
